...
8. If the package manager asks for a version number, enter the latest one, so `1.4.0`.
...
The entry `"com.i5.virtualagents": "1.4.0"` can be inserted anywhere in the dependencies object with the other installed packages that are already listed there.
...
3. Enter https://github.com/rwth-acis/Virtual-Agents-Framework.git#[version] into the text field where [version] is replaced with "v", followed by the release number, e.g. "v1.4.0" or upm for the latest version.
...
5. Click the import button to download the samples.
   They are saved in your Assets folder in a folder "Samples/Virtual Agents Framework/1.4.0".
